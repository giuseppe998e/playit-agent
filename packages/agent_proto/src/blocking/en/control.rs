...
impl MessageEncode for Pong {
    fn write_to<W: Write + ?Sized>(self, buf: &mut W) -> Result<()> {
        ...
        self.client_addr.write_to(buf)?;
        self.tunnel_addr.write_to(buf)?;
        self.session_expire_at.write_to(buf)
    }
}
