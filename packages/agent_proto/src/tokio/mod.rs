...
mod en_dec {
    ...
    use crate::{
        ...
        socket::{Port, Protocol, Socket, SocketFlow, SocketFlowV4, SocketFlowV6, V6_BYTES},
    };

    #[tokio::test]
    async fn test_agentsession() {
        ...
        // Encode
        assert!(matches!(data.clone().write_into(&mut buf).await, Ok(_)));

        // Decode
        let mut buf_cursor = Cursor::new(buf);
        let dec_result = AgentSession::read_from(&mut buf_cursor).await;
        assert_eq!(data, dec_result.unwrap())
    }

    #[tokio::test]
    async fn test_controlrequest() {
        ...
        // Decode
        let mut buf_cursor = Cursor::new(buf);
        let dec_result = ControlRequest::read_from(&mut buf_cursor).await;
        assert_eq!(data, dec_result.unwrap())
    }

    #[tokio::test]
    async fn test_controlresponse() {
        ...
        // Decode
        let mut buf_cursor = Cursor::new(buf);
        let dec_result = ControlResponse::read_from(&mut buf_cursor).await;
        assert_eq!(data, dec_result.unwrap())
    }

    #[tokio::test]
    async fn test_control_ping() {
        ...
        // Decode
        let mut buf_cursor = Cursor::new(buf);
        let dec_result = Ping::read_from(&mut buf_cursor).await;
        assert_eq!(data, dec_result.unwrap())
    }

    #[tokio::test]
    async fn test_control_pong() {
        ...
        // Decode
        let mut buf_cursor = Cursor::new(buf);
        let dec_result = Pong::read_from(&mut buf_cursor).await;
        assert_eq!(data, dec_result.unwrap())
    }

    #[tokio::test]
    async fn test_control_portmappingrequest() {
        ...
        // Decode
        let mut buf_cursor = Cursor::new(buf);
        let dec_result = PortMappingRequest::read_from(&mut buf_cursor).await;
        assert_eq!(data, dec_result.unwrap())
    }

    #[tokio::test]
    async fn test_control_portmappingresponse() {
        let rand = random();
        ...
        // Decode
        let mut buf_cursor = Cursor::new(buf);
        let dec_result = PortMappingResponse::read_from(&mut buf_cursor).await;
        assert_eq!(data, dec_result.unwrap())
    }

    #[tokio::test]
    async fn test_control_portmappingfound() {
        ...
        // Decode
        let mut buf_cursor = Cursor::new(buf);
        let dec_result = PortMappingFound::read_from(&mut buf_cursor).await;
        assert_eq!(data, dec_result.unwrap())
    }

    #[tokio::test]
    async fn test_control_registerrequest() {
        ...
        // Decode
        let mut buf_cursor = Cursor::new(buf);
        let dec_result = RegisterRequest::read_from(&mut buf_cursor).await;
        assert_eq!(data, dec_result.unwrap())
    }

    #[tokio::test]
    async fn test_control_registerresponse() {
        ...
        // Decode
        let mut buf_cursor = Cursor::new(buf);
        let dec_result = RegisterResponse::read_from(&mut buf_cursor).await;
        assert_eq!(data, dec_result.unwrap())
    }

    #[tokio::test]
    async fn test_control_udpchanneldetails() {
        ...
        // Decode
        let mut buf_cursor = Cursor::new(buf);
        let dec_result = UdpChannelDetails::read_from(&mut buf_cursor).await;
        assert_eq!(data, dec_result.unwrap())
    }
    ...
    async fn test_hmacsign() {
        ...
        // Decode
        let mut buf_cursor = Cursor::new(buf);
        let dec_result = HmacSign::<sha2::Sha256>::read_from(&mut buf_cursor).await;
        assert_eq!(data, dec_result.unwrap())
    }
    ...
    async fn test_socket() {
        ...
        // Decode
        let mut buf_cursor = Cursor::new(buf);
        let dec_result = Socket::read_from(&mut buf_cursor).await;
        assert_eq!(data, dec_result.unwrap())
    }

    #[tokio::test]
    async fn test_socket_port() {
        ...
        // Decode
        let mut buf_cursor = Cursor::new(buf);
        let dec_result = Port::read_from(&mut buf_cursor).await;
        assert_eq!(data, dec_result.unwrap())
    }

    #[tokio::test]
    async fn test_socket_proto() {
        ...
        // Decode
        let mut buf_cursor = Cursor::new(buf);
        let dec_result = Protocol::read_from(&mut buf_cursor).await;
        assert_eq!(data, dec_result.unwrap())
    }

    #[tokio::test]
    async fn test_socketflow_v4() {
        let mut buf = Vec::<u8>::with_capacity(V6_BYTES + 1);
        ...
        // Decode
        let mut buf_cursor = Cursor::new(buf);
        let dec_result = SocketFlow::read_from(&mut buf_cursor).await;
        assert_eq!(data, dec_result.unwrap())
    }
